...
meta:
  - name: description
    content: 學習使用ERP系統的创建模块数据。瀏覽、參考、範例程式碼、教學課程及更多資源。
  - name: keywords
    content: 创建模块数据, ERP, ERP software, ERP system, ERP solution.
---
...
| 名字              | 类型                   | 说明                                                  |
| ----------------- | ---------------------- | ------------------------------------------------------------ |
| authorization     | `String`(*Header*)     | **必填.** [通过 OAuth 获取](/pages/2ffa7b/)的访问令牌               |
| client_id         | `String`(*Header*)     | **必填.** aiM18 [授权应用列表](/pages/2ffa7b/)中的Client ID          |
...
如果找不到authorization和client_id，请查看 [此处](/pages/2ffa7b/)。
