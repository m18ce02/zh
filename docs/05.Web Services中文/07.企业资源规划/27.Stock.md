...
meta:
  - name: description
    content: 學習使用ERP系統的库存。瀏覽、參考、範例程式碼、教學課程及更多資源。
  - name: keywords
    content: 库存, ERP, ERP software, ERP system, ERP solution.
---
## 存货创始
...
| menuCode      | String(Query)  | 是    | os                                       |
| param         | String(Query)  | 否    | 额外的系统参数；JSON字符串                          |
| autoSubmit    | boolean(Query) | 否    | 是否允许自动送批，值为TRUE时自动送批                     |
...
| URL      | http://[server]/jsf/rfws/erp/bsFlow/save/ru |
| -------- | ---------------------------------------- |
| http请求方式 | POST                                     |
| 编码类型     | UTF-8                                    |
